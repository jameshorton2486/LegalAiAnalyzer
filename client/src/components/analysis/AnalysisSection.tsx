...
export function AnalysisSection({
  ...
  const transcriptId =
    selectedTranscriptId ||
    (transcripts && transcripts.length > 0 ? transcripts[0]?.id : undefined);
  ...
  const { data: analysisData, isLoading: isLoadingAnalysis } = useQuery({
    ...
    queryFn: () =>
      transcriptId
        ? fetchTranscriptAnalysis(transcriptId)
        : Promise.resolve([]),
  ...
  const questions =
    transcriptId && analysisData ? getQuestionsFromAnalysis(analysisData) : [];
  ...
  const runComparisonMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        ...
        description:
          "The AI is comparing transcripts. Results will be available soon.",
      });
  ...
  const handleRunComparison = () => {
    if (transcripts.length < 2) {
      toast({
        ...
        description:
          "You need at least two analyzed transcripts to run a comparison.",
        variant: "destructive",
      });
      return;
    }
    ...
    <div className="bg-white rounded-lg shadow-md p-6">
      ...
      <div>
            ...
            {isLoadingAnalysis ? (
              ...
              <div className="py-8 text-center border border-neutral-300 rounded-md">
                ...
                <p className="text-sm text-neutral-400 mt-2">
                  Analysis may still be in progress or no transcript has been
                  analyzed.
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                {questions.map((question: Question, index: number) => (
                  <div
                    key={index}
                    className="border border-neutral-300 rounded-md p-4 hover:bg-neutral-50"
                  >
              ...
              <div className="space-y-4">
                {contradictions?.map(
                  (contradiction: Contradiction, index: number) => {
                    const transcript1 = transcripts.find(
                      (t) => t.id === contradiction.transcript1Id,
                    );
                    const transcript2 = transcripts.find(
                      (t) => t.id === contradiction.transcript2Id,
                    );
                          ...
                          <div>
                            ...
                            <div className="mt-2 mb-3">
                              <div className="bg-neutral-50 p-3 rounded-md mb-2 text-sm">
                                <p className="font-medium text-primary-dark mb-1">
                                  {transcript1?.witnessName || "Witness 1"}:
                                </p>
                                <p className="text-neutral-600">
                                  {contradiction.excerpt1}
                                </p>
                              </div>
                              <div className="bg-neutral-50 p-3 rounded-md text-sm">
                                <p className="font-medium text-primary-dark mb-1">
                                  {transcript2?.witnessName || "Witness 2"}:
                                </p>
                                <p className="text-neutral-600">
                                  {contradiction.excerpt2}
                                </p>
                              </div>
                            </div>
                            <div className="flex space-x-3 text-sm">
                              <button
                                className="text-primary-dark hover:text-primary flex items-center"
                                onClick={() =>
                                  saveToNotes(contradiction.description)
                                }
                              >
                                <FaPlus className="mr-1" /> Save to notes
                              </button>
        ...
        {activeTab === "insights" && (
          <div className="py-8 text-center border border-neutral-300 rounded-md">
            <p className="text-neutral-500">
              Key insights feature coming soon.
            </p>
            <p className="text-sm text-neutral-400 mt-2">
              This feature will provide AI-generated insights about key themes
              and critical points in the transcript.
            </p>
          </div>
        )}
      </div>
