...
export async function fetchTranscriptAnalysis(transcriptId: number) {
  const response = await apiRequest("GET", `/api/transcripts/${transcriptId}/analysis`, undefined);
...
export function getQuestionsFromAnalysis(analysisItems: any[]) {
  const questionsAnalysis = analysisItems.find(item => item.type === "questions");
...
export function getInsightsFromAnalysis(analysisItems: any[]) {
  const insightsAnalysis = analysisItems.find(item => item.type === "insights");
...
export async function fetchCaseContradictions(caseId: number) {
  const response = await apiRequest("GET", `/api/cases/${caseId}/contradictions`, undefined);
  return await response.json();
}

// Initiate a comparison between transcripts
export async function compareTranscripts(caseId: number, transcriptIds: number[]) {
