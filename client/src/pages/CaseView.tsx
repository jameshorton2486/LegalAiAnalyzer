...
export default function CaseView() {
  ...
  const { data: transcripts = [], isLoading: isLoadingTranscripts } = useQuery<Transcript[]>({
    queryKey: [`/api/cases/${caseId}/transcripts`],
    enabled: !isNaN(caseId),
  });
  
  // Get analyzed transcripts for AI analysis
  const analyzedTranscripts = transcripts.filter(t => t.status === "analyzed");
  ...
  if (!caseData) {
    return (
      <Layout>
        <div className="text-center py-12">
          <h1 className="text-2xl font-semibold text-neutral-800 mb-2">Case Not Found</h1>
          <p className="text-neutral-600">The case you're looking for doesn't exist or you don't have permission to view it.</p>
        </div>
      </Layout>
    );
  }
  ...
  return (
    <Layout>
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-neutral-800">{caseData.title}</h1>
        <p className="text-neutral-600">
          {caseData.caseNumber ? `Case #${caseData.caseNumber}` : 'No case number'} • {caseData.description || 'No description'}
        </p>
      </div>
      ...
      <div className="mb-6 border-b border-neutral-300">
        <nav className="flex space-x-8">
          <a 
            href="#" 
            onClick={(e) => { e.preventDefault(); setActiveTab("transcripts"); }}
      ...
      {activeTab === "documents" && (
        <div className="bg-white rounded-lg shadow-md p-6 mb-6 text-center py-10">
          <h2 className="text-lg font-semibold text-neutral-800 mb-2">Documents</h2>
      ...
      {activeTab === "timeline" && (
        <div className="bg-white rounded-lg shadow-md p-6 mb-6 text-center py-10">
          <h2 className="text-lg font-semibold text-neutral-800 mb-2">Timeline</h2>
