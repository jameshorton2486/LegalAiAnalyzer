...
import { insertCaseSchema, insertTranscriptSchema, insertAnalysisSchema, insertContradictionSchema } from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Transcripts API
  app.get("/api/cases/:caseId/transcripts", async (req: Request, res: Response) => {
    try {
  ...
  app.post("/api/cases/:caseId/transcripts", upload.single("file"), async (req: Request, res: Response) => {
    try {
      if (!req.file) {
  ...
  // Analysis API
  app.get("/api/transcripts/:transcriptId/analysis", async (req: Request, res: Response) => {
    try {
  ...
  // Contradictions API
  app.get("/api/cases/:caseId/contradictions", async (req: Request, res: Response) => {
    try {
      const caseId = Number(req.params.caseId);
  ...
  app.post("/api/cases/:caseId/compare", async (req: Request, res: Response) => {
    try {
      const caseId = Number(req.params.caseId);
      const transcriptIds = req.body.transcriptIds;
      
      if (!Array.isArray(transcriptIds) || transcriptIds.length < 2) {
        return res.status(400).json({ error: "At least two transcript IDs are required" });
      }
      
      // This would be an async operation in practice
      compareTranscripts(caseId, transcriptIds).catch(console.error);
      
      res.status(202).json({ message: "Comparison initiated", status: "processing" });
    } catch (err) {
      handleError(err, res);
    }
  });
  ...
  try {
    ...
    for (let i = 0; i < transcripts.length; i++) {
      for (let j = i + 1; j < transcripts.length; j++) {
        ...
        const contradictions = await findContradictions(transcript1.content, transcript2.content);
  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are an expert legal assistant specialized in analyzing deposition transcripts. Your task is to identify areas where follow-up questions would be beneficial for an attorney conducting the deposition. Focus on inconsistencies, vague responses, potential contradictions, or areas where more detail would be helpful."
        },
        {
          role: "user",
  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are an expert legal assistant that analyzes deposition transcripts to identify key insights, themes, and important details that might be relevant to the case."
        },
        {
          role: "user",
  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are an expert legal assistant specialized in identifying contradictions between witness testimonies in depositions. Your task is to compare two deposition transcripts and highlight significant contradictions."
        },
        {
          role: "user",
